...
def main():
    ...
    controller = CascadedFlightController(
        mass=quad.mass, g=quad.g, L=quad.L, c_tf=quad.c_tf
    )
    ...
    waypoints = [
        ...
        (
            ...
            [1.0, 1.0, 1.5],
            90.0,
        ),  # Travel to X=1m, Y=1m, climb to 1.5m, rotate yaw to 90 deg
        (
            3.0,
            [0.0, 1.0, 1.2],
            180.0,
        ),  # Travel to X=0m, Y=1m, hover at 1.2m, face opposite side (180 deg)
    ...
    # 5. Run simulation
    # Total time = sum of all waypoint durations.
