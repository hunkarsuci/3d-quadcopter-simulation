...
def plot_flight_telemetry(history):
...
def animate_copter_3d(history, arm_length=0.25, save_path=None):
    ...
    # Physics is run at 500 Hz (dt=0.002). 500 / 15 steps ≈ 33 FPS (ideal for visualization)
    step_size = 15
    ...
    arm13_body = np.array([
        [l_arm, l_arm, 0],   # Motor 1
        [-l_arm, -l_arm, 0]   # Motor 3
    ]).T

    arm24_body = np.array([
        [l_arm, -l_arm, 0],  # Motor 2
        [-l_arm, l_arm, 0]   # Motor 4
    ]).T

    # Setup plotting components
    arm13_line, = ax.plot([], [], [], 'r-o', linewidth=3, markersize=8, label="Front-Left / Rear-Right")
    arm24_line, = ax.plot([], [], [], 'b-o', linewidth=3, markersize=8, label="Front-Right / Rear-Left")
    ...
    ani = animation.FuncAnimation(
        fig, update_frame, frames=len(states), init_func=init,
