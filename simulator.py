...
class QuadcopterSimulator:
    ...
    def __init__(self, quadcopter: Quadcopter, controller: CascadedFlightController):
        ...
        # Waypoints queue: list of dicts: {'time_end': float, 'pos': [x, y, z], 'yaw': float}
        self.waypoints = []
    ...
    def get_current_waypoint(self, current_time):
        ...
        while (self.current_wp_idx < len(self.waypoints) - 1 and
               current_time > self.waypoints[self.current_wp_idx]['time_end']):
    ...
    def run(self, total_time=20.0, dt=0.002, control_frequency=250):
        ...
        print(f"Starting Drone simulation: Total Time: {total_time}s, Physics Step dt: {dt}s, Control Rate: {control_frequency}Hz...")

        for step in range(n_steps):
            # 1. Update waypoint targets
            target_pos, target_yaw = self.get_current_waypoint(current_time)

            # 2. Run controller at decimated frequency (closer to real-world embedded systems)
            ...
            self.history['control'].append(control_log.copy() if 'control_log' in locals() else {})
